                    ...
                    if (context.InputParameters.Contains("Target") && context.InputParameters["Target"] is Entity)
                    {
                        ...
                        if (entTraget.Contains("directioncode") && !((Boolean)entTraget["directioncode"]))
                        {
                            ...
                            if (entTraget.Contains("regardingobjectid"))
                            {
                                ...
                                {
                                    trace.Trace("Ok");
                                    Entity entApproval = service.Retrieve(objID.LogicalName, objID.Id, new ColumnSet("ownerid", "spectra_feasibility"));
                                    if (entTraget.Contains("from") && entApproval.Attributes.Contains("spectra_feasibility"))
                                    {
                                        trace.Trace("from");
